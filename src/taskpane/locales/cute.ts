...
export const cuteText: ThemeTextMap = {
  ...
  manage: {
    ...
    addChannelHint: '添加世界后点击「同步世界」为所有宝典补充世界列~',
    gitDirLabel: '魔法书架位置',
    gitDirPlaceholder: '魔法书架位置（必填）',
    channelNamePlaceholder: '世界名称',
    statusChannelAdded: (name, field) => `世界「${name}」(${field}) 已开启~`,
    statusChannelDeleted: (name) => `世界「${name}」已关闭~`,
    statusSyncResult: (n) => `路线同步完成: ${n} 本宝典已更新~`,
    statusTableCreated: (name) => `宝典「${name}」制作成功！`,
    variableHint: '{0}=章节.冒险编号 {1}=世界名 {2}=冒险者',
  },
  ...
  help: {
    ...
    exportSection: {
      title: '出发冒险',
      flow: '出发页面有 3 个小窗口（出发 | 成果 | 日记）。选择世界 → 设定宝物存放处 → 点击出发。小精灵会自动加载配置、筛选数据、通过哈希魔法对比变化，只整理有变动的宝典~',
      outputDir: '在「小窝设置」中配置。支持变量替换：{0} = 章节号，{1} = 世界名。',
      git: '出发完成后，如果魔法书架可用，精灵会自动帮你整理和上架~ 咒语模板支持参数：{0}=章节.冒险编号，{1}=世界名，{2}=冒险者。提交时会使用当前冒险者的名字哦~ 成果页有上架按钮和丢弃按钮~',
    },
    collab: {
      title: '组队冒险',
      howItWorks: '通过 StudioConfig 工作表实现小伙伴们一起冒险~ 冒险者在 StudioConfig 中填写世界、章节，并在「冒险者」栏写入自己的名字就能触发出发。组队出发会强制自动上架到魔法书架，不受本地设置影响哦~',
      monitor: '守护精灵默认开启，每 5 秒巡逻一次。状态：绿色 = 守护中~，蓝色 = 帮忙出发中，灰色 = 休息中。',
    },
    manageSection: {
      title: '小窝管理',
      config: '管理冒险者、世界管理（世界名 + 路线）、小伙伴名册（支持删除）、魔法书架位置（全局配置，不是每个世界一个）、魔法卷轴（{0}=章节.冒险编号，{1}=世界名，{2}=冒险者）和功能开关。开辟新世界后记得点击「同步路线」~',
      tableManage: '数据来源为「宝典目录」工作表，直接在 Excel 中编辑即可实时同步。',
      newTable: '通过制作间创建新宝典，自动登记到宝典目录并添加快捷链接~',
    },
    validateSection: {
      title: '守护检查',
      intro: '对选中的宝典执行守护检查，检测咒语格式、数据类型、必填字段等问题。检查范围和规则会自动记住，下次打开时恢复哦~ 点击结果可自动找到问题所在~',
    },
    previewSection: {
      title: '冒险预览',
