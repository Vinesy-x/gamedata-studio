...
export const pixelText: ThemeTextMap = {
  ...
  manage: {
    ...
    addChannelHint: '开启新副本后点击「同步副本」为所有精灵补充副本列',
    gitDirLabel: '传送阵坐标',
    gitDirPlaceholder: '传送阵坐标（必填）',
    channelNamePlaceholder: '副本名称',
    statusChannelAdded: (name, field) => `副本「${name}」(${field}) 已开启`,
    statusChannelDeleted: (name) => `副本「${name}」已关闭`,
    statusSyncResult: (n) => `路线同步完成: ${n} 个精灵已更新`,
    statusTableCreated: (name) => `精灵「${name}」锻造成功!`,
    variableHint: '{0}=副本编号.回合 {1}=副本名 {2}=勇者',
  },
  ...
  help: {
    ...
    exportSection: {
      title: '过关',
      flow: '过关界面有 3 个子页（执行 | 战果 | 日志）。选择副本 → 设定宝箱位置 → 点击 GO! 系统自动加载配置、筛选数据、通过哈希对比自动识别变更，仅输出有变化的精灵。',
      outputDir: '在「营地」中配置。支持变量替换：{0} = 副本编号，{1} = 副本名。',
      git: '过关完成后，如果传送阵可用，系统会自动传送。传送咒语支持参数：{0}=副本编号.回合，{1}=副本名，{2}=勇者。提交时使用当前勇者名称。战果页提供传送按钮和丢弃按钮。',
    },
    collab: {
      title: '多人组队',
      howItWorks: '通过 StudioConfig 工作表实现多人组队过关。勇者在 StudioConfig 中填写副本、编号，并在「勇者」栏写入自己的名字触发过关。组队过关会强制自动传送到传送阵，不受本地设置影响。',
      monitor: '巡逻哨兵默认开启，以 5 秒间隔巡逻。状态：绿色 = 巡逻中，蓝色 = 正在过关，灰色 = 休息中。',
    },
    manageSection: {
      title: '营地管理',
      config: '管理勇者、副本管理（副本名 + 路线）、冒险队伍（支持删除）、传送阵坐标（全局配置，非按副本）、传送咒语（{0}=副本编号.回合，{1}=副本名，{2}=勇者）和功能开关。开启新副本后点击「同步路线」。',
      tableManage: '数据来源为「精灵图鉴」工作表，直接在 Excel 中编辑即可实时同步。',
      newTable: '通过锻造炉创建新精灵，自动收录到图鉴并添加快捷链接。',
    },
    validateSection: {
      title: '鉴定',
      intro: '对选中的精灵执行鉴定检查，检测副本格式、数据类型、必填字段等问题。鉴定范围和规则选择会自动保存到本地存储，下次打开时恢复。点击结果可自动定位到问题单元格。',
    },
    previewSection: {
      title: '探索',
