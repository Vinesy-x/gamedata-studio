...
export const cyberText: ThemeTextMap = {
  ...
  help: {
    ...
    exportSection: {
      title: '上传',
      flow: '上传界面包含 3 个子面板（执行 | 报告 | 日志）。选择频道 → 配置上传节点 → 点击上传。系统自动加载配置、筛选数据、通过哈希校验自动识别变更，仅上传有变化的数据体。',
      outputDir: '在「终端」中配置。支持变量替换：{0} = 协议版本，{1} = 频道名。',
      git: '上传完成后，如果暗网节点可用，系统会自动推送。推送脚本支持参数：{0}=协议版本.序列号，{1}=频道名，{2}=黑客。提交作者使用当前黑客身份。报告页提供推送按钮和丢弃按钮。',
    },
    collab: {
      title: '协同上传',
      howItWorks: '通过 StudioConfig 工作表实现多人协同上传。黑客在 StudioConfig 中填写频道、协议版本，并在「黑客」栏写入自己的名字触发上传。协同上传会强制自动推送到暗网节点，不受本地设置影响。',
      monitor: '入侵检测默认开启，以 5 秒间隔扫描。状态：绿色 = 监听中，蓝色 = 协同上传中，灰色 = 监听关闭。',
    },
    manageSection: {
      title: '终端管理',
      config: '管理黑客、频道管理（频道名 + 链路）、黑客名册（支持删除）、暗网路径（全局配置，非按频道）、推送脚本（{0}=协议版本.序列号，{1}=频道名，{2}=黑客）和功能开关。添加新频道后点击「同步链路」。',
      tableManage: '数据来源为「数据体目录」工作表，直接在 Excel 中编辑即可实时同步。',
      newTable: '通过编译器创建新数据体，自动注册到目录并添加快捷链接。',
    },
    validateSection: {
      title: '扫描',
      intro: '对选中的数据体执行扫描规则，检测协议格式、数据类型、必填字段等问题。扫描范围和规则选择会持久化到本地存储，下次启动时自动恢复。点击结果可自动定位到异常单元格。',
    },
    previewSection: {
      title: '解码',
