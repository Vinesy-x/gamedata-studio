...
export const defaultText: ThemeTextMap = {
  ...
  manage: {
    ...
    variableHint: '{0}=版本号.序列号 {1}=渠道名 {2}=操作员',
  },
  ...
  help: {
    ...
    exportSection: {
      title: '导出',
      flow: '导出页面包含 3 个子页（导出 | 结果 | 日志）。选择渠道 → 设置输出目录 → 点击导出。系统自动加载配置、筛选数据、通过哈希对比自动识别变更，仅导出有变化的表。',
      outputDir: '在「管理」选项卡中配置。支持变量替换：{0} = 版本号，{1} = 渠道名。',
      git: '导出完成后，如果本地文件服务器正在运行，系统会自动执行 Git add、commit、push。Git 提交模板支持参数：{0}=版本号.序列号，{1}=渠道名，{2}=操作员。提交作者使用当前操作员名称。结果页提供推送按钮和丢弃按钮。',
    },
    collab: {
      title: '协同导出',
      howItWorks: '通过 StudioConfig 工作表实现多人协同导出。操作员在 StudioConfig 中填写输出渠道、版本号，并在「操作员」栏写入自己的名字触发导出。协同导出会强制自动 Git 推送，不受本地设置影响。',
      monitor: '默认开启，以 5 秒间隔轮询 StudioConfig 表。状态指示：绿色 = 监听中，蓝色 = 正在协同导出，灰色 = 已关闭。',
    },
    manageSection: {
      title: '管理',
      config: '管理操作员、渠道管理（渠道名 + 线路）、人员代码（支持删除）、Git 目录（全局配置，非按渠道）、Git 提交模板（{0}=版本号.序列号，{1}=渠道名，{2}=操作员）和功能开关。添加新渠道后点击「同步线路」为所有数据表补充对应的 roads 列。',
      tableManage: '数据来源为「表名对照」工作表，直接在 Excel 中编辑表名对照即可实时同步。',
      newTable: '通过向导创建符合规范的数据表工作表，自动注册到表名对照并添加超链接。',
    },
    validateSection: {
      title: '校验',
      intro: '对选中的数据表执行校验规则，检测版本区间格式、数据类型、必填字段、Roads 一致性等问题。校验范围和规则选择会自动保存到本地存储，下次打开时恢复。点击校验结果可自动定位到问题单元格。',
    },
    previewSection: {
      title: '预览',
