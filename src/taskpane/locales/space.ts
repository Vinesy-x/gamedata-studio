...
export const spaceText: ThemeTextMap = {
  ...
  manage: {
    ...
    variableHint: '{0}=航线编号.航班号 {1}=航道名 {2}=舰长',
  },
  ...
  help: {
    ...
    exportSection: {
      title: '发射',
      flow: '发射界面包含 3 个子面板（发射 | 报告 | 日志）。选择航道 → 设定着陆坐标 → 点击发射。系统自动加载配置、筛选数据、通过哈希校验自动识别变更，仅输出有变化的设备。',
      outputDir: '在「舰桥」中配置。支持变量替换：{0} = 航线编号，{1} = 航道名。',
      git: '发射完成后，如果本地传送站正在运行，系统会自动执行星际传送（add、commit、push）。传送协议支持参数：{0}=航线编号.航班号，{1}=航道名，{2}=舰长。提交作者使用当前舰长名称。报告页提供传送按钮和丢弃按钮。',
    },
    collab: {
      title: '协同发射',
      howItWorks: '通过 StudioConfig 工作表实现多人协同发射。舰长在 StudioConfig 中填写航道、航线编号，并在「舰长」栏写入自己的名字触发发射。协同发射会强制自动星际传送，不受本地设置影响。',
      monitor: '默认开启，以 5 秒间隔扫描 StudioConfig 表。状态指示：绿色 = 扫描中，蓝色 = 协同发射中，灰色 = 雷达关闭。',
    },
    manageSection: {
      title: '舰桥',
      config: '管理舰长、航道管理（航道名 + 航段）、船员编制（支持删除）、传送坐标（全局配置，非按航道）、传送协议（{0}=航线编号.航班号，{1}=航道名，{2}=舰长）和功能开关。添加新航道后点击「同步航段」为所有设备补充对应的航段列。',
      tableManage: '数据来源为「设备清单」工作表，直接在 Excel 中编辑设备清单即可实时同步。',
      newTable: '通过向导组装新设备，自动注册到设备清单并添加超链接。',
    },
    validateSection: {
      title: '维修',
      intro: '对选中的设备执行检修规则，检测航线区间格式、数据类型、必填字段、航段一致性等问题。检修范围和规则选择会自动保存到本地存储，下次打开时恢复。点击检修结果可自动定位到故障单元格。',
    },
    previewSection: {
      title: '试飞',
